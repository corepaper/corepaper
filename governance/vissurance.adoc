...
[meta=description]
Issurance design has been one of the primary things that governance
system of a blockchain has to decide. Issurance comes in varities of
...
different issurance bodies to co-exist within a blockchain, and to
allow them to be created and followed at will. If no issurance curve
can last for a hundred years, then do not have a fixed one.

== Design

A blockchain following voluntary issurance does not have a fixed
...
Genesis block includes a transaction that deploys the first issurance
body on-chain.
