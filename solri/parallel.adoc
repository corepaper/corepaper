...
For example, a process `A` may invoke a child process `B` but return early
in current block, and only inspect `B`'s status till the next
block. It makes no difference if `B` is actually executed in the
