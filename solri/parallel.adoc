...
early in current block, and only inspect the status of `B` status till
the next block. It makes no difference if `B` is actually executed in
the current block or the next block, and based on the runtime
executor's business, it can determine that itself.
